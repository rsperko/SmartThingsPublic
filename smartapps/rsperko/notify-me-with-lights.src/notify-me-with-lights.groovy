 ...
 *
 *  Notify Me With Lights
 *
 *  Author: SmartThings
 *  Date: 2014-01-20
 */
definition(
    name: "Notify Me With Lights",
    namespace: "rsperko",
    author: "Rick Sperko",
    description: "Blinks the lights and brightness of bulbs when any of a variety of SmartThings is activated.  Supports motion, contact, acceleration, moisture and presence sensors as well as switches.",
    category: "My Apps",
...
preferences {
	...
	section("Control these bulbs...") {
		input "lights", "capability.switchLevel", title: "Which Bulbs?", required:true, multiple:true
	}
...
private takeAction(evt) {
	...
	def newValue = [level: (lightLevel as Integer) ?: 100]
	log.debug "new value = $newValue"

	lights*.setLevel(newValue.level)
	setTimer()
}
...
def setTimer() {
	if(!duration) { //default to 10 seconds
		...
		pause(duration * 1000)
		log.debug "resetLights"
		resetLights()
	}
	else {
		log.debug "runIn $duration, resetLights"
		runIn(duration,"resetLights", [overwrite: false])
	}
}
...
def resetLights() {
	lights.each {
        ...
        if("on" == state.previous[it.id].switch) {
        	it.on()
        }
        else {
        	it.off()
